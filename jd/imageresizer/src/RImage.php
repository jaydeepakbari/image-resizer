...
class RImage extends Model{
    ...
    public static function resize($filename,$height,$width){
		$DIR_IMAGE = public_path('/');
		...
		if (!is_file($DIR_IMAGE . $image_new) || (filemtime($DIR_IMAGE . $image_old) > filemtime($DIR_IMAGE . $image_new))) {
			...
			if ($width_orig != $width || $height_orig != $height) {
				$image = new ImageTool($DIR_IMAGE . $image_old);
				$image->resize($width, $height);
				$image->saveImage($DIR_IMAGE . $image_new);
			} else {
...
class ImageTool extends Model{
	...
	public function saveImage($file, $quality = 90) {
		$info = pathinfo($file);
